import { BlogPost, parseMarkdown } from "@/lib/blog";
...
export default async function BlogPostContent({ post, language }: BlogPostContentProps) {
                ...
                <div className="max-w-4xl mx-auto flex justify-between items-center">
                    <Link href="/blog" className="flex items-center gap-2 text-white/40 hover:text-white transition-colors group">
                        ...
                        <span className="text-xs font-bold uppercase tracking-widest">
                            {language === 'en' ? 'Back to Insights' : 'Volver a Noticias'}
                        </span>
                    </Link>
                ...
                <div className="mb-16">
                    <div className="flex items-center gap-3 mb-8">
                        ...
                        <span className="text-[10px] text-white/30 uppercase tracking-[0.2em] font-mono">
                            {new Date(post.date).toLocaleDateString(language === 'en' ? 'en-US' : 'es-ES', { month: 'long', day: 'numeric', year: 'numeric' })}
                        </span>
                    </div>
                    ...
                    <div className="flex items-center gap-4 py-8 border-y border-white/5">
                        ...
                        <div>
                            ...
                            <div className="text-[10px] text-white/40 uppercase tracking-tighter">
                                {language === 'en' ? 'NavAI Editorial Team' : 'Equipo Editorial NavAI'}
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div className="relative z-10 flex flex-col md:flex-row items-center gap-12">
                        <div className="flex-1 space-y-6 text-center md:text-left">
                            <h3 className="text-2xl md:text-3xl font-bold">
                                {language === 'en' ? 'Ready to try the Web Bridge Assistant?' : '¿Listo para probar el Asistente de Puente Web?'}
                            </h3>
                            <p className="text-white/60 font-light">
                                {language === 'en'
                                    ? 'Access our professional maritime tools directly from your browser. Use the Bridge Console web app or get the mobile version for iOS.'
                                    : 'Acceda a nuestras herramientas marítimas profesionales directamente desde su navegador. Utilice la consola web o descargue la versión para iOS.'}
                            </p>
                            <div className="flex flex-wrap gap-4 justify-center md:justify-start">
                                ...
                                >
                                    ...
                                    <span>{language === 'en' ? 'Open Web Bridge for Free' : 'Abrir Puente Web Gratis'}</span>
                                </Link>
                                <a
                                ...
                                >
                                    ...
                                    <span>{language === 'en' ? 'Get iOS App for Free' : 'Obtener App iOS Gratis'}</span>
                                </a>
                            </div>
                        </div>
